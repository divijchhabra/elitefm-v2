...
// example: Image.asset('images/camera.png',)
  AssetImage playy =  AssetImage('assets/playicons.png',);
  AssetImage pausee =  AssetImage('assets/pauseicon.png',);
  ...
  void getName() async{
    ...
    assert(time is int);
    print(time);
    final studio = FirebaseFirestore.instance.collection('studio').doc(DateFormat('EEEE').format(date)).collection("schedule");
    QuerySnapshot query =
    await studio.where('starttime', isLessThanOrEqualTo:time).get();
    for(var element in query.docs ){
      if((element.data() as dynamic)["starttime"]<=time && (element.data() as dynamic)["endtime"] > time ){
        setState(() {
  ...
  updateUrl()  async{
    FirebaseFirestore.instance.collection('liveAudio').doc('audio').get().then((value) {
      setState(() {
  ...
  Text getvolume(){
    print(currentVolume);
    if(currentVolume<0.1){
      return Text('${currentVolume*100}'.substring(0,1) ,
      ...
      return Text('${currentVolume*100}'.substring(0,2) ,
        style: TextStyle(color: whiteColor, fontSize: 45),);
    }
  }
                                  ...
                                  child: SleekCircularSlider(
                                          ...
                                          child: Container(
                                            ...
                                            decoration: BoxDecoration(
                                              image: DecorationImage(
                                                  ...
                                                  image:   justplayer.playerState.playing?AssetImage('assets/pauseicon.png'):AssetImage('assets/playicons.png'),
                                                  fit: justplayer.playerState.playing? BoxFit.contain:BoxFit.contain
                                              ),
                                            ),
                                    ...
                                    appearance: CircularSliderAppearance(
                                      ...
                                      customWidths: CustomSliderWidths(
                                        ...
                                        handlerSize: 10, // thing we select to drag progress
                                        shadowWidth: 10,
                                      ),
